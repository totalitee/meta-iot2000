...
class SoftwareSettings:
	...
	def setRepo(self):
		ret = EntryWindow(
			self.topmenu.gscreen,
			"Please enter a valid address for a opk package repository(ip or name only).",
			"",
			[("Host Address:", "")],
			1,
			70, 50,
	...
	def changePackages(self):
		...
		removeList = ''
		if (bb.buttonPressed(result) != "ok" or numberOfRemovablePackages > 0):
			return;
...
class Peripherals:
	...
	def show(self):
		menuItems = [("Configure External Com Ports", self.configureComPorts)]
		while(True and not self.finish):
			action, selection = ListboxChoiceWindow(
				self.topmenu.gscreen, 
				"", "", 
